...
where
     ...
     AND m.p_id = d.p_id
     AND m.client_id = 83
     AND d.date_ENTRY >=  to_date('1-feb-2011','dd-MON-yyyy')
     AND d.date_ENTRY < to_date('1-apr-2011','dd-MON-yyyy')
     and lower(m.p_title) like '%report%'
group by m.p_id
...
where lower(client_name) like '%ign%'
and lower(dba_id)
order by 2;
